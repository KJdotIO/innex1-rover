...
class StartZoneLocaliser(Node):
    ...
    def _on_odometry(self, msg: Odometry) -> None:
        """Track planar yaw so the search remains bounded in angle."""
    ...
    def _on_tag_pose(self, msg) -> None:
    ...
    def _stop_search_motion(self) -> None:
        """Publish one stop command when relinquishing the search path."""
    ...
    def _reset_after_clock_jump(self, now_ns: int) -> None:
        """Clear transient state after ROS time starts late or jumps."""
    ...
    def _handle_fresh_lock_window(self, now_ns: int) -> None:
        """Track a candidate lock and signal readiness once stable."""
        ...
        if candidate_elapsed_ns >= int(
            self.candidate_settle_duration_s * 1e9
    ...
    def _handle_missing_lock_window(
        self, now_ns: int, latest_sample: PoseSample | None
    ) -> None:
        """Continue the bounded search when no stable lock is available."""
        self.candidate_entered_ns = None
    ...
    def _handle_lock_tracking(self, now_ns: int) -> None:
        """Advance the candidate-lock state machine from tag samples."""
    ...
    def _on_timer(self) -> None:
        ...
        if (
            self.last_timer_now_ns is None
            or now_ns < self.last_timer_now_ns
            or self.search_started_ns is None
        ):
