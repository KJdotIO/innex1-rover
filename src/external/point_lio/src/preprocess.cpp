...
void Preprocess::sim_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    ...
    const auto point_count = static_cast<int>(pl_orig.size());
    ...
    for (int i = 0; i < point_count; i++) {
        ...
        // Gazebo's generic PointCloud2 bridge does not preserve Ouster-style
        // per-point timing. For simulation, keep a monotonic zero-offset scan
        // so the backend can run on the plain XYZI cloud shape.
