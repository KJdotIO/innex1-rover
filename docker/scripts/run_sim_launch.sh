...
CONTAINER_NAME="${CONTAINER_NAME:-innex1-sim}"
DETACH="${DETACH:-false}"
...
"${docker_cmd[@]}" "${docker_args[@]}" \
  ...
  -e SERVER_ONLY="${SERVER_ONLY}" \
  "${gpu_runtime_args[@]}" \
  -v "$(pwd):/workspaces/innex1-rover" \
  -w /workspaces/innex1-rover \
  "${IMAGE_NAME}" \
  bash -lc "if [[ ! -f install/setup.bash ]]; then echo 'Workspace is not built yet. Run ./docker/scripts/run_sim_dev.sh and colcon build --symlink-install first.' >&2; exit 1; fi; ros2 launch lunabot_simulation moon_yard.launch.py headless_rendering:=${HEADLESS_RENDERING} server_only:=${SERVER_ONLY}"
